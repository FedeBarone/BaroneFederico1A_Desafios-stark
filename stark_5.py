...
def imprimir_menu_desafio_5()-> None:# 1.1
...
def stark_menu_principal_desafio_5()-> None:# 1.2
    return_aux = -1
    imprimir_menu_desafio_5()
    opcion = input("Ingrese opcion: ")
    if re.match("[A-OZa-oz]{1}$", opcion):
        return_aux = opcion
    return return_aux
...
def stark_marvel_app_5(lista_heroes: list)-> None:# 1.3
    while True:
        os.system("cls")
...
def leer_archivo(archivo_json: str)-> list:# 1.4
    lista_heroes = []
...
def obtener_nombre_capitalizado(heroe: dict)-> str:# 1.7
    nombre_formateado = f"Nombre: {capitalizar_palabras(heroe['nombre'])}"
